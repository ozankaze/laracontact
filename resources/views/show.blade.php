  ...
  <body>
        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                  <h4>Hobby List : {{ $contact->name }}
                    <a onclick="addForm()" class="btn btn-primary pull-right" style="margin-top: -8px;">Add Hobby</a>
                  </h4>
                </div>
                <div class="panel-body">
                    <table id="hobbies-table" class="table table-striped">
                        <thead>
                            <tr>
                                <th width="30">No</th>
                                <th>Hobby</th>
    ...
    <script type="text/javascript">
      var table = $('#hobbies-table').DataTable({
                      processing: true,
                      serverSide: true,
                      ajax: "{{ route('api.hobbies.contact', $contact->id) }}",
                      columns: [
                        {data: 'id', name: 'id'},
      ...
      function editForm(id) {
        ...
        $('#modal-form form')[0].reset();
        $.ajax({
          url: "{{ url('hobby') }}" + '/' + id + "/edit",
          type: "GET",
          dataType: "JSON",
          success: function(data) {
            $('#modal-form').modal('show');
            $('.modal-title').text('Edit Hobby');
      ...
      $(function(){
            $('#modal-form form').validator().on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('hobby') }}";
                    else url = "{{ url('hobby') . '/' }}" + id;
